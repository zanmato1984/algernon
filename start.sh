...
if [ -z $1 ]; then
  ...
  if [ -z $2 ]; then
    ...
    docker-compose -f docker-compose-go-ycsb.yaml run go-ycsb0 load mysql -P /go-ycsb.conf -p mysql.host=tidb-raft0 -p mysql.port=4000 -p mysql.user=root
    docker-compose -f docker-compose-go-ycsb.yaml run go-ycsb0 run mysql -P /go-ycsb.conf -p mysql.host=tidb-raft0 -p mysql.port=4000 -p mysql.user=root
  else
    print_usage
    exit 1
  fi
